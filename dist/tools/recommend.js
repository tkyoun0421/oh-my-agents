...
export function registerRecommendTool(server) {
    server.tool("skills_recommend", "프로젝트를 분석하고 작업 의도를 기반으로 유용한 스킬을 추천합니다", {
        projectPath: z.string().describe("분석할 프로젝트 경로"),
        intent: z
            .string()
            .optional()
            .describe("현재 작업 의도 (예: 'TDD로 API 개발', '디자인 시스템 구축')"),
        ...
        const installedNames = new Set(installed.flatMap(({ skills }) => skills.map((s) => s.name)));
        ...
        const header = [
            ...
            .filter((l) => l !== null)
            .join("\n");
        const items = top5.map((s, i) => `${i + 1}. **${s.name}** (${s.installs.toLocaleString()} installs)\n   추천 이유: \`${s.reason}\` 관련\n   설치: \`skills_install "${s.source}"\``);
        return {
