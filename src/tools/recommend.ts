    ...
    async ({ projectPath, intent }) => {
      ...
      const stackInfo = [
        context.frameworks.length
          ? `기반 기술: ${context.frameworks.join(", ")}`
          : null,
        context.languages.length
          ? `개발 언어: ${context.languages.join(", ")}`
          : null,
        context.architecturePatterns.length
          ? `감지된 패턴: ${context.architecturePatterns.join(", ")}`
          : null,
      ]
        .filter(Boolean)
        .join(" / ");
      ...
      const header = [
        ...
        `**분석 결과**: ${stackInfo || "감지된 기술 스택 및 패턴 없음"}`,
        intent ? `**작업 의도**: ${intent}` : null,
        `프로젝트의 아키텍처와 기술 스택을 분석하여 상호 보완적인 고성능 도구들을 선별했습니다.`,
        "",
      ]
        .filter((l) => l !== null)
        .join("\n");
      ...
      const items = top5.map(
        (s, i) =>
          [
            `${i + 1}. **${s.name}**`,
            `   - **📊 설치 지수**: ${s.installs.toLocaleString()} installs (검증된 인기도)`,
